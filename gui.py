...
def add_p_core():
    ...
    if p_core + e_core < 4:
        ...
        # text 입력
        text = Text(processor_frame, height=3, padx=300)
...
def add_process():
    ...
    if process < 15:
        ...
        process_frame.grid(column = 0, row = process)

        # PROCESS 이름
        PROCESS = Label(process_frame, text=("P", process+1), width=10, bg=color[process])
...
def delete_process():
    global process
    if process > 0:
        process_root_frame.grid_slaves().pop(0).destroy()
        process -= 1
...
def clear():
    ...
    global e_core
    global core
    for frame in process_root_frame.grid_slaves():
        frame.destroy()
...
# CLEAR BUTTON
btn_run = Button(input_frame, padx=5, pady=5, width=5, text="CLEAR", bg="red")
